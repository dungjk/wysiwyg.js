var wysiwyg = (function () {
    ...
    var getSelectedNodes = function (containerNode) {
        ...
        for (var i = 0; i < sel.rangeCount; ++i) {
            var range = sel.getRangeAt(i), node = range.startContainer, endNode = range.endContainer;
            while (node) {
                // add this node?
    ...
    var getSelectionHtml = function (containerNode) {
        ...
        var sel = window.getSelection();
        if (sel.rangeCount) {
            var container = document.createElement("div"), len = sel.rangeCount;
            for (var i = 0; i < len; ++i) {
    ...
    var selectionInside = function (containerNode, force) {
        ...
        var sel = window.getSelection();
        if (isOrContainsNode(containerNode, sel.anchorNode) &&
            isOrContainsNode(containerNode, sel.focusNode)) {
            return true;
        }
    ...
    var wysiwyg = function (element, options) {
        ...
        var nodeContainer = typeof element == "string" ? document.querySelector(element) : element;
        ...
        // initialize editor
        var nodeContenteditable = nodeContainer.querySelector("[contenteditable=true]");
        ...
        var add_class_focus = function () {
            ...
            if (toolbar_demand)
                addClass(nodeContainer, "focused");
        };
        ...
        top) {
            ...
            var viewport_width = MathMin(window.innerWidth, MathMax(documentElement.offsetWidth, documentElement.scrollWidth));
            ...
            if (rect.left + left < 1)
                ...
                left = MathMax(1 - rect.left, viewport_width - 1 - rect.left - popup_width);
            if (rect.top + top < 1) {
                top = 1 - rect.top;
            }
            else if (rect.top + top + popup_height > viewport_height - 1) {
                top = MathMax(1 - rect.top, viewport_height - 1 - rect.top - popup_height);
            }
            // Set offset
        ...
        var fill_buttons = function (toolbar_container, selection_rect, buttons, hotkeys) {
            buttons.forEach(function (button) {
                ...
                if ("attr" in button) {
                    Object.keys(button.attr).forEach(function (k) {
                ...
                if ("click" in button) {
                    ...
                    handler = function () {
                        switch (button.action) {
                            ...
                            case "colortext":
                                if (selection_rect)
                                    open_popup_selection(selection_rect, "colortext", createColorpalette, true);
                                else
                                    open_popup_button(element, "colortext", createColorpalette, true);
                                break;
                            case "colorfill":
                                if (selection_rect)
                                    open_popup_selection(selection_rect, "colorfill", createColorpalette, false);
                                else
                                    open_popup_button(element, "colorfill", createColorpalette, false);
                                break;
                            case "subscript":
            ...
            for (var i = 0; i < nodes.length; ++i) {
                ...
                var closest = node.closest || // latest
                    function (selector) {
                        ...
                        var node = this;
                        while (node) {
                            var matchesSelector = node.matches ||
        ...
        var syncTextarea = null, debounced_syncTextarea = null;
        if (node_textarea) {
        ...
        if (onSelection) {
            var handleSelection = function (clientX, clientY, rightclick) {
                ...
                var rect = clientX === null || clientY === null
                    ? null
                    : {
                        left: clientX,
                        top: clientY,
        ...
        var popupOpen = function () {
            ...
            var parent = nodeContenteditable.parentNode, next = nodeContenteditable.nextSibling;
            if (next) {
        ...
        var keyHandler = function (e, phase) {
            ...
            var key = e.which || e.keyCode, character = String.fromCharCode(key || e.charCode), shiftKey = e.shiftKey || false, altKey = e.altKey || false, ctrlKey = e.ctrlKey || false, metaKey = e.metaKey || false;
            if (phase == 1) {
                // Callback
                if (onKeyDown &&
                ...
                // Callback
                if (onKeyPress &&
                    onKeyPress(key, character) === false)
                    cancelEvent(e); // dismiss key
            }
            //else if( phase == 3 )
            //{
        ...
        commands = {
            ...
            getSelectedHTML: function () {
                restoreSelection(nodeContenteditable, popup_saved_selection);
                if (!selectionInside(nodeContenteditable))
                    return null;
            ...
            openPopup: function () {
                if (!popup_saved_selection)
                    popup_saved_selection = saveSelection(); // save current selection
                return popupOpen();
            },
